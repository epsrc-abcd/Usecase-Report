\subsection{SMTP Client State Machine}
...
\begin{center}
\scalebox{1}{
\begin{statemachine}[node distance=4.8cm]
	\node[initial,state]		(S0)							{\lstinline|S0|};
	\node[state]				(S1)[right of=S0]				{\lstinline|S1|};
	\node[state]				(S2)[below =2cm of S1]		{\lstinline|S2|};
	\node[state]				(S3)[left of=S2]				{\lstinline|S3|};
	\node[state]				(S29)[below = 4cm of S3]		{\lstinline|S29|};
	\node[state]				(S32)[right of = S29]				{\lstinline|S32|};
	\node[state]				(S30)[above = 2cm of S32]				{\lstinline|S30|};
	\node[state]				(S31)[below = 2cm of S32]		{\lstinline|S31|};
	
	\node[state]				(S39)[right of = S1]			{\lstinline|S39|};
	\node[state,accepting]		(END)[below =2cm of S39]		{\lstinline|END|};
	\node[state]				(SX)[below = 4cm of END]		{\lstinline|Other|};
