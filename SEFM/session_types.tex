...
It also presents the Scribble~\cite{scribble} %\footnote{Scribble Project homepage, \url{www.scribble.org}"}
...
the notion of a type to capture communication behaviour. % in concurrent systems.
...
%Session types can also support the case of {\em session delegation}
...
The case where the implementation of all the communicating
...
of type \Code{title} labelled with label \Code{book} is sent \lstinline[basicstyle=\normalsize\ttfamily]|from|
...
%Scribble offers a tool-chain that implements
most of the procedures described in \figref{diagram}.
...
a global protocol (top of \figref{diagram})
...
% whose subsequent behaviour depends on them.
...
%One way to use the set of local protocols is
to ensure
...
%the {\em communication safety} and {\em communication progress}
%properties of session types.

\myparagraph{Code generation:}
Local protocols can be used 
%A second way to use the set of local protocols is
...
each local protocol. %one of the set of Scribble local protocols.
...
%of the global protocol guarantees that the
