...
The results in \secref{usecases} come to support
...
e.g.~the SMTP client state machine in \figref{put_figure}.
...
For example the SMTP global protocol (\figref{put_figure}) (resp., HTTP)
...
\mybf{
	Future (Maybe this can go as section 5)
\begin{itemize}
	\item	There is no evaluation of the bottom
			up technique due to the current lack of technologies.
			Nevertheless, this work is a strong indicator 
			that whenever session fidelity is ensured, either
			...
			This can help more agile development teams in
			the process of exchange requirements and co-ordinating
			software development.


	\item	A whole new research challenge for software development and
			session types.
	
	\item	Specific research in the benefits of session types
			in software engineering

	\item	Development time, Bugs, Lines of code, performance
	
	\item	cost
	
	\item	Session types have the ability to capture the
			communication inside a system as a whole and then project it to
			a local level.
			They further provide support through the different phases of
			software abstractions.
			It would be useful to research whether session types can capture
			the communication in software that requires more complicate
			structures and more level of abstraction.
\end{itemize}
}
