...
%	\tikzset{myshape/.style={draw, minimum width = 1cm, minimum height = 0.5cm}}
	\tikzset{myshape/.style={draw, circle, minimum size = 0.5cm}}
%	\tikzset{noshape/.style={draw}}
	...
	% Verify
	\node[myshape, below = 0.5cm of ScribbleIn]		(Verify)				{};
	\draw[->]		(ScribbleIn) -- node[left] {\scriptsize verify} (Verify);

	% Local Protocols
	\node[below = 0.75cm of Verify]					(LocalC)				{\dots};
	\node[myshape, left = 0cm of LocalC]			(LocalL)				{};
	\node[myshape, right = 0cm of LocalC]			(LocalR)				{};

	\node[below = 0.5cm of Verify, minimum width = 2cm, minimum height = 1cm]	(Local)	{};
	\node[below = 0 of LocalC]											{\scriptsize Local Protocols};
	
	% TODO fix this
	\draw[->]		(Verify)	--	node[left]	{\scriptsize project}	(LocalL);
	\draw[->]		(LocalR)	--	node[right]	{\scriptsize synthesis}	(Verify);

	% Typecheck
	\node[vertrec, left = 1cm of Local]				(Exec)					{};
	\node[above = 0 of Exec]												{\scriptsize Verified Code};
	\node[left = 1.25cm of Local]					(ExecC)					{\vdots};
	\node[myshape, above= 0cm of ExecC]				(ExecT)					{};
	\node[myshape, below= 0cm of ExecC]				(ExecB)					{};
	\draw[->]	(Local)	-- node[above] {\scriptsize typecheck}	(Exec);
...
%	\draw[<->]	(MonC) -- (RunC);
	...
	% Code
	\node[below = 1cm of Local]						(Code)					{\dots};
	\node[myshape, left = 0cm of Code]				(CodeL)					{};
	\node[myshape, right = 0cm of Code]				(CodeR)					{};
	\node[below = 0.1 of Code]												{\scriptsize Source Code};
